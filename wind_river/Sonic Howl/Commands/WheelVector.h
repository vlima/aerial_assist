...
public:
   ...
   {
      if (xabs(leftFront) < value)
      {
         leftFront = 0.0;
      }
      if (xabs(leftBack) < value)
      {
         leftBack = 0.0;
      }
      if (xabs(rightFront) < value)
      {
         rightFront = 0.0;
      }
      if (xabs(rightBack) < value)
      {
         rightBack = 0.0;
      }
   }

   void correct(WheelVector &error, WheelVector &delta, double min)
   {
      if (error.leftFront < min)
      {
         leftFront += delta.leftFront;
      }
      if (error.leftBack < min)
      {
         leftBack += delta.leftBack;
      }
      if (error.rightFront < min)
      {
         rightFront += delta.rightFront;
      }
      if (error.rightBack < min)
      {
         rightBack += delta.rightBack;
      }
   }
